...
- When designing or updating UI headers in a CLI tool, align textual elements (like title and version) with visual elements (like ASCII art or separators) by integrating them into the same layout structure, and use consistent separator lines to enhance visual coherence and professionalism, as seen in the updated header layout in src/ui.js.
- When adjusting UI output in a CLI tool, ensure proper spacing by adding newlines before status messages or other critical output to enhance readability, as seen in the update to src/ui.js where a newline was added before the status message.
- When handling build processes in a CLI tool, capture both stdout and stderr outputs separately during execution to provide detailed error information for debugging, as seen in the updates to `runBuild` in src/git.js.
- When displaying error messages in a CLI tool's UI, implement a dedicated method (like `printHeaderWithError`) to format and color-code detailed error outputs distinctly (e.g., in red) to improve user feedback, as seen in the addition to src/ui.js.
- In a CLI tool's main execution flow, ensure that build or critical process failures are caught and their detailed error outputs (if available) are displayed using a specialized UI method before exiting, as seen in the updates to `main()` in src/index.js.
<!-- TURL-RULES-END -->
